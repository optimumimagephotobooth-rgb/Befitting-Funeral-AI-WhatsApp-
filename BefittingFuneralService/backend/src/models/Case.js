...
import { cloneComplianceTemplateForCase } from '../services/complianceAssignmentService.js';
...
export class Case {
  static async create(contactId, data = {}) {
    ...
    const result = await pool.query(
      `INSERT INTO cases (
        contact_id, case_ref, status, stage, deceased_name, funeral_date, 
        location, package_name, total_amount, deposit_amount, compliance_template_id, compliance_template_version
      ...
      [
        ...
        data.compliance_template_id || null,
        data.compliance_template_version || null
      ]
    );
    const createdCase = result.rows[0];

    if (createdCase?.id && data.compliance_template_id) {
      await cloneComplianceTemplateForCase(
        createdCase.id,
        data.compliance_template_id,
        data.compliance_template_version || 'v1'
      );
    }
