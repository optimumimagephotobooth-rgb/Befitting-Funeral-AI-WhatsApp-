...
export class AnalyticsService {
  ...
  async getForecastDashboard() {
    try {
      const latestForecasts = await pool.query(`
        SELECT DISTINCT ON (fe.case_id)
          fe.case_id,
          fe.risk_score,
          fe.level,
          fe.summary,
        ...
        ORDER BY fe.case_id, fe.created_at DESC
      `);

      const distribution = await pool.query(`
        SELECT level, COUNT(*)::integer as count
        FROM (
          SELECT DISTINCT ON (case_id) case_id, level
      ...
      const recentHints = await pool.query(`
        SELECT ce.case_id, ce.metadata, ce.created_at, c.case_ref
        FROM case_events ce
        JOIN cases c ON c.id = ce.case_id
        WHERE ce.event_type = 'SUPERVISOR_HINT'
        ORDER BY ce.created_at DESC
        LIMIT 20
      `);

      return {
        latestForecasts: latestForecasts.rows,
        distribution: distribution.rows,
        recentEvents: recentEvents.rows,
        recentHints: recentHints.rows
      };
    } catch (error) {
      console.error('Error getting forecast dashboard data:', error);
      throw error;
    }
  }
